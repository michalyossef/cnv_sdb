...
//       $scope.data.powerInfo.push({"core": "tmp_core", clusters:[], primitives: []});
	...
	$rootScope.loadingState = true;
	httpService.get('power/tree/' + $scope.unitsLatestReleaseId,{'parent': node.instance},
			function(res){
...
// 			    }
	...
	if(node.isUnit || node.release_id == "virtual"){
	  ...
	  $scope.data.selectedCluster.units.push(node);
	...
	for(var i=0;i<$scope.data.selectedCluster.units.length;i++){
	  if(child._id == $scope.data.selectedCluster.units[i]._id){
	    pos = i;
	    break;
	  }
	}
	if(pos == -1){
	  ...
	  $scope.data.selectedCluster.units.push(child);
    ...
    $scope.editUnitsOnPrimitiveChange = function(){
      ...
      httpService.get('power/released/' + $scope.data.selectedProject + '/' + $scope.data.selectedInfo.core.name + '/'  + $scope.data.selectedCluster.name + '/' + $scope.data.tempUnitPrimitiveChange.name + '/rtls',{},
		      function(res){
			  $rootScope.loadingState = false;
